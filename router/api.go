...
func SendByRoute(b []byte, routeName string) (tickets []data.OutputResponse, err error) {
	return Instance().sendByRoute(parseBytes(b), routeName)
}

// SendByRoute sends the input message to a route with retry on the output send
func SendMsgByRoute(msg map[string]interface{}, routeName string) (tickets []data.OutputResponse, err error) {
